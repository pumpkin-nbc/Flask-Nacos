...
`NACOS_SERVICE_NAME` 与 `NACOS_SERVICE_PORT` 为必填项。若未提供
`NACOS_SERVICE_IP`，将自动检测本机 IP。
...
失败时的行为由 `NACOS_FAIL_FAST` 控制：

- `NACOS_FAIL_FAST = False`（默认）：Nacos 初始化、注册、配置读取失败时仅记录日志，
  不会阻止 Flask 应用启动。服务发现 / 配置读取方法会返回安全默认值（`None` / `[]`）。
...
- `NacosRegistrationError` —— 注册 / 注销失败。
