...
def test_version_check_passes_with_current_version(check_version):
    ...
    assert ok, message
    assert versions["pyproject.toml"] == "1.0.2"
    assert versions["flask_nacos/__init__.py"] == "1.0.2"
    assert versions["CHANGELOG.md"] == "1.0.2"
    assert versions["CHANGELOG.zh-CN.md"] == "1.0.2"
...
def test_validate_wheel_names_requires_license_and_notice(check_package):
    names = [
        ...
        "flask_nacos/extension.py",
        "flask_nacos/py.typed",
        "flask_nacos-1.0.2.dist-info/METADATA",
    ]
...
def test_release_tag_must_match_version(check_release_tag):
    assert check_release_tag.validate_tag("v1.0.2") == "v1.0.2"
